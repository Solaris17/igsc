...
#define gsc_debug(_fmt_, ...) \
    if (igsc_get_log_level() >= IGSC_LOG_LEVEL_DEBUG) { \
        if (NULL == igsc_get_log_callback_func()) { \
            debug_print(_fmt_, ##__VA_ARGS__); \
        } else { \
            igsc_log_func_t igsc_log_func = igsc_get_log_callback_func(); \
            igsc_log_func(IGSC_LOG_LEVEL_DEBUG, _fmt_, ##__VA_ARGS__); \
        } \
    }
...
#define gsc_trace(_fmt_, ...) \
    if (igsc_get_log_level() >= IGSC_LOG_LEVEL_TRACE) { \
        if (NULL == igsc_get_log_callback_func()) { \
            trace_print(_fmt_, ##__VA_ARGS__); \
        } else { \
            igsc_log_func_t igsc_log_func = igsc_get_log_callback_func(); \
            igsc_log_func(IGSC_LOG_LEVEL_TRACE, _fmt_, ##__VA_ARGS__); \
        } \
    }

#define gsc_error(_fmt_, ...) \
    if (NULL == igsc_get_log_callback_func()) { \
        error_print(_fmt_, ##__VA_ARGS__); \
    } else { \
        igsc_log_func_t igsc_log_func = igsc_get_log_callback_func(); \
        igsc_log_func(IGSC_LOG_LEVEL_ERROR, _fmt_, ##__VA_ARGS__); \
    }
